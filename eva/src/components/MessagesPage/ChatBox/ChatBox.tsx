...
export default function ChatBox({ profile_image, profile_name, id, last_message, last_connection }: ChatBoxProps) {
    ...
    const goToChat = () => {
        // create the socket for this chat
        chat.addNewSocket(id)
		if (id) {
			history.push(`${id}`);
		}
	};
	...
	const isMessageSeen = useCallback(
		() => {
			...
			console.log(
				'id->',
				idToCheck,
				last_connection[idToCheck],
				last_message.creation_date,
				last_connection[idToCheck] >= last_message.creation_date
			);
	...
	return (
		<div
			className={'chat__box ' + getExtraClasses(seen, isSmallScreen)}
			onMouseEnter={() => setIsOptionsShown(true)}
			onMouseLeave={() => setIsOptionsShown(false)}
			onClick={() => goToChat()}
		>
			<div
				className={isSmallScreen ? 'chat__box__profile__picture_sm' : 'chat__box__profile__picture'}
				title={profile_name}
			>
			...
			<div className={isSmallScreen ? 'hidden' : 'not_hidden'}>
				<div className="chat__box__main__info">
					...
					<div className="chat__box__last__message__info">
						<div className={!seen ? 'chat__box__last__message bold_text' : 'chat__box__last__message'}>
							{last_message.content.length > 26 ? (
								`${last_message.content.substring(0, 26)}...`
							) : (
				...
				<div className="chat__box__message__info">
					<div className="chat__box__message__info__status">
						{!seen &&
						!isOptionsShown && (
							<FiberManualRecordIcon className="chat__box__message__info__status__button" />
						)}
						{seen &&
						!isOptionsShown && (
							<img className="chat__box__message__info__status__img" src={profile_image} alt="" />
						)}
